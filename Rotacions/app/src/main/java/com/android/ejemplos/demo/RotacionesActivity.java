...
public class RotacionesActivity extends Activity {
    ...
    protected void onSaveInstanceState(Bundle outState){
        ...
        outState.putSerializable(DATA, mRowData);
    }

    @Override
    protected void onRestoreInstanceState(Bundle savedInstanceState) {
        super.onRestoreInstanceState(savedInstanceState);
        ArrayList<InfoPago> newRowData =
                (ArrayList<InfoPago>) savedInstanceState.getSerializable(DATA);
        if (newRowData != null) {
            mRowData = newRowData;
            for(InfoPago info: newRowData) {
                addRow(info);
            }
        }
    }
